 ...
 * analog DemoPvpGame, aber mit dem echten Board (DiceDuelBoard/DiceDuelEnd).
 */
...
export function DemoDiceDuelGame({
  engine,
  gameName,
  engineConfig,
  verifierUrl,
  ...
  const [match, setMatch] = useState<DemoDiceDuelView | null>(null);
  ...
  const start = useCallback(async () => {
    ...
    try {
      ...
      }).then((x) => x.json())) as DemoDiceDuelView & JsonErr;
      if (r.error) {
